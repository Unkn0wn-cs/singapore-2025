...
void loop() {
    ...
    if (!(sensorValues[0] || sensorValues[1] || sensorValues[2] || sensorValues[3]) && (back == false)) // no line detected and going forward
    {
        motor1.run(BACKWARD);
        motor1.setSpeed(200);
        motor2.setSpeed(200);
        ...
        if (sensorValues[0] && sensorValues[1] && not (sensorValues[3]))
        {
            motor1.run(FORWARD);
            motor2.run(BACKWARD);
        ...
        else if (sensorValues[2] && sensorValues[3] && not (sensorValues[1]))
        {
            motor1.run(BACKWARD);
            motor2.run(FORWARD);
            motor2.setSpeed(baseSpeed * 2);
